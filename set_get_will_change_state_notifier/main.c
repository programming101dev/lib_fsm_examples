...
int main(void)
{
    ...
    else
    {
        ...
        notifier_func = p101_fsm_info_get_will_change_state_notifier(env, fsm);
        printf("Before set: 0x%lx\n", (uintptr_t)notifier_func);

        p101_fsm_info_set_will_change_state_notifier(env, fsm, notifier);
        notifier_func = p101_fsm_info_get_will_change_state_notifier(env, fsm);
        printf("After set: 0x%lx\n", (uintptr_t)notifier_func);

        p101_fsm_info_set_will_change_state_notifier(env, fsm, NULL);
        notifier_func = p101_fsm_info_get_will_change_state_notifier(env, fsm);
