...
static void notifier(const struct p101_env *env, struct p101_error *err, const struct p101_fsm_info *info, p101_fsm_state_id from_state_id, p101_fsm_state_id to_state_id);
...
int main(void)
{
    ...
    {
        p101_fsm_info_bad_change_state_notifier_func notifier_func;

        notifier_func = p101_fsm_info_get_bad_change_state_notifier(env, fsm);
        printf("Before set: 0x%lx\n", (uintptr_t)notifier_func);

        p101_fsm_info_set_bad_change_state_notifier(env, fsm, notifier);
        notifier_func = p101_fsm_info_get_bad_change_state_notifier(env, fsm);
        printf("After set: 0x%lx\n", (uintptr_t)notifier_func);

        p101_fsm_info_set_bad_change_state_notifier(env, fsm, NULL);
        notifier_func = p101_fsm_info_get_bad_change_state_notifier(env, fsm);
...
static void notifier(const struct p101_env *env, struct p101_error *err, const struct p101_fsm_info *info, p101_fsm_state_id from_state_id, p101_fsm_state_id to_state_id)    // cppcheck-suppress constParameterCallback
{
    P101_TRACE(env);
}
